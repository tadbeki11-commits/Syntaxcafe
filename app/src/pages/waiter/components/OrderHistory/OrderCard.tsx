...
interface OrderCardProps {
  order: any;
  addingItemsOrderId: number | null;
  addingItems: any[];
  menuItems: any[];
...
export const OrderCard: React.FC<OrderCardProps> = ({
  ...
  // Dynamic aesthetic backgrounds depending on status
  const getAmbientStyles = () => {
    switch (effectiveStatus) {
      ...
      case 'preparing':
        return 'from-blue-500/5 via-transparent to-transparent border-info/30/40 hover:border-blue-400/50';
      case 'ready':
      case 'paid':
      case 'completed':
        return 'from-emerald-500/5 via-transparent to-transparent border-emerald-200/40 hover:border-emerald-400/50';
      case 'cancelled':
        return 'from-red-500/5 via-transparent to-transparent border-destructive/30/40 hover:border-red-400/50';
      default:
  ...
  const getSpotlightColor = () => {
    switch (effectiveStatus) {
      ...
      case 'preparing': return 'bg-info/10';
      case 'ready':
      case 'paid':
  ...
  return (
    <Card className={`relative overflow-hidden border bg-gradient-to-br from-card to-card/95 hover:shadow-[0_12px_45px_rgba(0,0,0,0.025)] hover:-translate-y-0.5 transition-all duration-300 rounded-[28px] group ${getAmbientStyles()}`}>
      
      {/* Decorative ambient background spotlight glow */}
      <div className={`absolute top-0 right-0 w-36 h-36 rounded-full blur-3xl group-hover:scale-125 transition-transform duration-500 pointer-events-none ${getSpotlightColor()}`} />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border/40">
          <div className="flex items-start gap-3.5">
            <div className="shrink-0">
              <Badge variant={statusDisplay.variant} className="rounded-2xl w-11 h-11 p-0 flex items-center justify-center border-none shadow-sm group-hover:scale-110 transition-transform duration-300">
                <StatusIcon className="w-5.5 h-5.5 animate-pulse" />
              </Badge>
            </div>
            <div>
          ...
          <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 shrink-0">
            <Badge variant={statusDisplay.variant} className="text-[9px] uppercase font-black tracking-wider py-0.5 px-2.5 border-none shadow-sm">
              {statusDisplay.label}
            </Badge>
        ...
        {order.items && order.items.length > 0 && (
          <div className="space-y-2.5">
            <div className="flex items-center gap-1.5">
              ...
              <h4 className="text-[10px] uppercase font-black text-muted-foreground tracking-wider">Checkout Items Detailed List</h4>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
              {order.items.map((item: any, idx: number) => (
                <div key={idx} className="text-xs text-foreground bg-muted/20 border border-border/40 hover:border-primary/15 hover:bg-muted/30 transition-all rounded-2xl p-3 flex justify-between items-center shadow-inner group/item">
                  <span className="truncate pr-2 font-bold flex items-center gap-1.5">
                    <span className="font-black text-primary bg-primary/10 px-2 py-0.5 rounded-lg text-[10px] shrink-0 group-hover/item:scale-105 transition-transform">{item.quantity}x</span> 
                    <span className="truncate">{item.menu_item_name}</span>
                  </span>
                  <span className="font-extrabold text-muted-foreground shrink-0">{formatCurrency(item.subtotal)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {order.notes && (
          <div className="border-t border-border/40 pt-4">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <h4 className="text-[10px] uppercase font-black text-muted-foreground tracking-wider">Kitchen Instructions Notes</h4>
            </div>
