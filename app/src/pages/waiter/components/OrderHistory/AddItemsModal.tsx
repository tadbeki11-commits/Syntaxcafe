  ...
  const filteredCatalog = useMemo(() => {
    return menuItems.filter(item => {
      ...
      if (catalogSearch) {
        ...
        const matchesCategory = String(item.category || item.sub_category || '').toLowerCase().includes(query);
      ...
      <DialogContent className="max-w-4xl rounded-[32px] overflow-hidden p-6 gap-6 bg-gradient-to-b from-background via-background to-background/98 shadow-2xl border-white/20">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription className="text-xs font-bold text-muted-foreground/80 mt-0.5">
                Modify table checkout, takeaway, or beu tickets dynamically by selecting catalog items.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 my-2">
          ...
          <div className="md:col-span-7 flex flex-col gap-3.5">
            <div className="flex items-center justify-between">
              <h5 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Cafe Menu Catalog</h5>
                ...
                filteredCatalog.map((menuItem) => (
                  ...
                  >
                    <div className="min-w-0 flex-1 truncate pr-2">
                      <p className="font-extrabold text-xs text-foreground truncate">{menuItem.name}</p>
                      <p className="text-[9px] text-muted-foreground font-bold uppercase mt-0.5 tracking-wider">
                        {menuItem.main_category || 'unit'}
                        {(menuItem.sub_category || menuItem.category) ? ` • ${menuItem.sub_category || menuItem.category}` : ''}
                      </p>
                    </div>
                    <Badge variant="secondary" className="text-[10px] font-black border bg-muted/60 shrink-0">
          ...
          <div className="md:col-span-5 flex flex-col gap-3.5 border-t md:border-t-0 md:border-l pt-5 md:pt-0 md:pl-5">
            <div className="flex items-center justify-between">
              <h5 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Slated Additions</h5>
            ...
            {addingItems.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-6 border border-dashed rounded-2xl bg-muted/10 min-h-[220px]">
                ...
                <p className="text-xs font-bold text-muted-foreground">No additions slated yet</p>
                <p className="text-[10px] text-muted-foreground/70 max-w-[160px] mt-1 mx-auto leading-relaxed">
                  Tap any product card in the menu catalog on the left to add it here.
                </p>
              </div>
            ) : (
              <div className="flex-1 flex flex-col justify-between gap-4">
                ...
                <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1 flex-1">
                  {addingItems.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-background border border-border/70 p-3 rounded-2xl text-xs shadow-sm hover:shadow transition-all duration-300">
                      <div className="min-w-0 flex-1 truncate pr-2">
                        <p className="font-extrabold text-foreground truncate">{item.menu_item_name}</p>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <Button
                          ...
                          onClick={() => updateAddingItemQuantity(idx, item.quantity - 1)}
                        >
                          <Minus className="w-3 h-3" />
                        </Button>
                        <span className="w-4 text-center font-black text-xs text-foreground">{item.quantity}</span>
                        <Button
                          variant="outline"
                          size="icon"
                          className="h-6 w-6 rounded-lg hover:border-primary/25 bg-background shadow-sm"
        ...
        <DialogFooter className="border-t pt-4">
          <div className="flex gap-2 w-full justify-end">
            ...
            >
              <Save className="w-3.5 h-3.5 text-primary-foreground" /> Save Additions
            </Button>
          </div>
        </DialogFooter>
