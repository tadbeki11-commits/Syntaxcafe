    ...
    <div className="flex flex-col overflow-hidden bg-card h-full min-h-0">
      ...
      <div className="shrink-0 border-b border-border/60 bg-gradient-to-r from-amber-200 via-amber-100 to-amber-100 px-5 py-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="uppercase tracking-[0.22em] text-warning">Order Summary</p>
            <h3 className="mt-1  font-extrabold tracking-tight text-foreground">Current table order</h3>
         
            <div className="mt-3">
            {selectedTable ? (
              <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-700 border-none font-bold py-0.5 px-2 rounded-lg text-[9px] flex items-center gap-1 shrink-0 shadow-sm">
                <MapPin className="w-3 h-3 text-emerald-500 animate-bounce" />
                {selectedTable === 'takeaway' ? 'Take Away' : selectedTable === 'beu' ? 'Beu' : `Table #${selectedTable}`}
              </Badge>
            ) : (
              <Badge variant="secondary" className="bg-warning/10 text-amber-700 border-none font-bold py-0.5 px-2 rounded-lg text-[9px] flex items-center gap-1 shrink-0 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 rounded-2xl border border-border/70 bg-background/80 px-3 py-1.5 shadow-sm">
              <span className="text-[10px] font-black uppercase tracking-wider text-foreground">{totalItems} items</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 py-5 space-y-4">
        {orderItems.length === 0 ? (
          <div className="text-center space-y-3 rounded-[28px] border border-dashed border-border/80 bg-gradient-to-b from-muted/10 to-background p-5 py-16 animate-in fade-in duration-300">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-wider text-foreground">Your cart is empty</p>
              <p className="mx-auto mt-1 max-w-[220px] text-[10px] font-semibold leading-relaxed text-muted-foreground/75">
                Add items from the menu to build the table order and submit it when ready.
              </p>
            </div>
          </div>
        ) : (
      ...
      <div className="shrink-0 px-5 pb-5">
        ...
        <Button
          disabled={orderItems.length === 0 || creatingOrder || tableRequirementUnmet}
