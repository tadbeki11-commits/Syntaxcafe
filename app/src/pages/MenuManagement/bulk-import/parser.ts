...
export interface ParsedMenuRow {
  /** 1-based row number from the source file (header excluded) for error messages. */
  rowNumber: number;
  name: string;
  description: string;
...
const csvToRawRows = (text: string): RawRow[] => {
  ...
  return dataRows.map((cells) => {
    ...
    fieldByIndex.forEach((field, idx) => {
      if (field) (raw as any)[field] = cells[idx];
    });
    return raw;
  });
};
  ...
  const list = Array.isArray(parsed)
    ...
    : Array.isArray(parsed?.menuItems)
      ...
      : Array.isArray(parsed?.items)
        ...
        : null;
  if (!list) {
    throw new Error("JSON must be an array of items (or an object with a menuItems array).");
  }
  return list.map((entry: any) => {
    const raw: RawRow = {};
    for (const key of Object.keys(entry || {})) {
      const field = mapHeaderToField(key);
      if (field) (raw as any)[field] = entry[key];
    }
    return raw;
  });
};
...
const normalizeRow = (
  ...
  const mainCategory = slugifyCategory(str(raw.main_category)) || "barista";
  if (str(raw.main_category) && !ctx.knownMainCategorySlugs.has(mainCategory)) {
    warnings.push(`Unknown department "${mainCategory}" — it will be saved as-is`);
  }
  ...
  if (lowerName) {
    if (seenNames.has(lowerName)) {
      ...
      warnings.push("An item with this name already exists in the menu");
    }
...
export const parseMenuFile = (
  ...
  try {
    ...
    return { rows: [], fatalError: e?.message || "Failed to parse file" };
  }
