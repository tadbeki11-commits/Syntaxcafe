// Device enrollment for the web waiter portal.
//
// Mirrors the desktop app's device-token flow, but persists in localStorage
// instead of SQLite. On first run an operator redeems a one-time enrollment
// code (minted by an owner/platform admin for a branch) via POST /devices/enroll
// for a long-lived token. That token is stored here and attached as the
// `x-device-token` header on every waiter API request so the backend pins this
// device to its branch (see backend TenantMiddleware).
 ...
 * it. After this resolves, every subsequent waiter API call carries the device's
