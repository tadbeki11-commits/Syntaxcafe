...
export const useWaiterCreateOrder = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (resolvedMenuItems.length === 0) {
        const fallbackMenuResult = await Promise.allSettled([api.menu.getAll()]);
  ...
  const isFastingCategory = useCallback((_cat: any) => {
    return true;
  }, []);
  ...
  const handleSubmitOrder = useCallback(async () => {
    ...
    try {
      ...
      if (forceTableSelection && !isTableSelected) {
        toast.error(
          "Table selection is mandatory. Take Away and Beu are not allowed.",
        );
        return;
      }
      ...
      // Shared device: return to the login screen so the next waiter signs in.
      logout();
      router.push("/waiter/login");
    } catch (error: any) {
  ...
  const waiterName =
    user?.full_name || (user as any)?.name || user?.username || "Waiter";
