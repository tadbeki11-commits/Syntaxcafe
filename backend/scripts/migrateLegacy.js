/*
 * Generic migration: an old single-tenant cafe DB -> an EXISTING business+branch
 * in the new multi-tenant DB. Does NOT create business/branch/owner (they exist).
 * Remaps integer PKs/FKs to UUIDs and stamps business_id/branch_id on every row.
 *
 * Usage:
 *   SRC_DB=luxury1_db BUSINESS_ID=<uuid> BRANCH_ID=<uuid> node scripts/migrateLegacy.js
 *   DRY=1 SRC_DB=... BUSINESS_ID=... BRANCH_ID=... node scripts/migrateLegacy.js
 */
...
if (!SRC_DB || !BUSINESS_ID || !BRANCH_ID) {
  throw new Error("SRC_DB, BUSINESS_ID and BRANCH_ID env vars are required");
}
...
async function main() {
  ...
  // sanity: business + branch must exist and match
  const chk = await tgt.query(
    "SELECT 1 FROM branches WHERE id=$1 AND business_id=$2",
    [BRANCH_ID, BUSINESS_ID],
  );
  if (!chk.rowCount) {
    throw new Error("Branch not found for that business in target. Aborting.");
  }
  // abort if branch already has data
  const used = await tgt.query(
    "SELECT count(*)::int n FROM orders WHERE branch_id=$1",
    [BRANCH_ID],
  );
  if (used.rows[0].n > 0) {
    throw new Error(
      `Branch already has ${used.rows[0].n} orders. Aborting to avoid duplicates.`,
    );
  }
  ...
  if (DRY) {
    console.log(`\nDRY run for ${SRC_DB} -> branch ${BRANCH_ID}. No writes.`);
    await src.end();
    await tgt.end();
    return;
  }
  ...
  try {
    ...
    console.log(`\nCOMMITTED ${SRC_DB} -> business ${BUSINESS_ID} branch ${BRANCH_ID}.`);
  } catch (e) {
