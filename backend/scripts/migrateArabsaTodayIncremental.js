 ...
 * single-tenant arabsa1_db (5461) into the ALREADY-migrated Arabsa
 * business+branch in the multi-tenant production DB (5450).
 *
 ...
 *
 * Usage:
 *   DRY=1 node scripts/migrateArabsaTodayIncremental.js   (read + validate, no writes)
 *       node scripts/migrateArabsaTodayIncremental.js      (execute in one txn)
 */
...
const BUSINESS_ID = "a5aa0fb0-a87e-4749-8e08-bca4d7945cf1";
const BRANCH_ID = "6f2576e5-d837-4269-8b80-88ff717a770a";
...
async function main() {
  ...
  if (!chk.rowCount) throw new Error("Arabsa branch/business not found in target.");
  ...
  // natural keys verified unique on both sides (except menu name dups, which are
  // not referenced today; we still guard against ambiguity below).
  const buildMap = async (table, key) => {
    const sRows = await q(`SELECT id, ${key} k FROM ${table}`);
    const tRows = (
      await tgt.query(
        `SELECT id, ${key} k FROM ${table} WHERE business_id=$1`,
        [BUSINESS_ID],
      )
    ).rows;
  ...
  try {
    ...
    console.log(`\nCOMMITTED ${DAY} top-up -> Arabsa branch ${BRANCH_ID}.`);
  } catch (e) {
