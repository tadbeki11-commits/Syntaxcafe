/*
 * One-off migration: arabsa1_db (old single-tenant) -> new multi-tenant DB.
 * Creates Business "Arabsa", Branch "Branch1", a new owner (arabsa1), migrates
 * all staff + operational data, remapping integer PKs/FKs to UUIDs and stamping
 * business_id/branch_id on every row.
 *
 * Usage: node scripts/migrateArabsa.js            (dry plan + execute)
 *        DRY=1 node scripts/migrateArabsa.js       (read source, print counts, no writes)
 */
...
const SRC =
  "postgres://postgres:donXYhApLQ2aKkjFgewOdeRaIiB1HEs1SbCvE6Ck5gkatka1hyFEBswB2c8XdOLD@37.60.230.104:5461/arabsa1_db";
const TGT =
...
const OWNER_USERNAME = "arabsa1";
const OWNER_PASSWORD = "arabsa123";
...
async function main() {
  ...
  if (exists.rowCount) {
    throw new Error("Business slug 'arabsa' already exists in target. Aborting.");
  }
  ...
  // new owner (cross-branch: branch_id NULL)
  usersRows.push({
    id: OWNER_ID,
  ...
  const orderRows = S.orders.map((o) => ({
    ...
    organization_id: null, // no organizations in source
  ...
  if (DRY) {
    console.log("\nDRY run — no writes. Built row counts:");
    console.log({
  ...
  // ---- insert helper (chunked multi-row) ----
  ...
  try {
    await tgt.query(
      ...
      [BUSINESS_ID, "Arabsa", "arabsa", OWNER_ID, "standard", true],
    );
    await tgt.query(
      `INSERT INTO branches (id,business_id,name,slug,is_active) VALUES ($1,$2,$3,$4,$5)`,
      [BRANCH_ID, BUSINESS_ID, "Branch1", "branch1", true],
    );
    console.log("inserted business + branch");

    await ins(
      "users",
      ["id", "business_id", "branch_id", "name", "username", "password_hash", "pin_hash", "role", "role_id", "first_name", "last_name", "phone", "is_active", "cancel_password", "print_copies", "meta", "created_at", "updated_at"],
      usersRows,
    );
